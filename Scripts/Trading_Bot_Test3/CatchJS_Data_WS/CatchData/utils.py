...
def extract_payload(console_text: str, prefix: str):
    """Return (ok, payload) where payload is parsed JSON (dict/list) or the raw tail."""
...
def is_divergence_event(p: dict) -> bool:
    """Accept normalized 'aggr/indicator' events that include L1 & L2."""
    if not isinstance(p, dict):
        return False
    if p.get("v") != 1 or p.get("source") != "aggr/indicator":
        return False
    if not isinstance(p.get("thread_id"), str):
        return False
    if not isinstance(p.get("sequence"), int):
        return False
    if not isinstance(p.get("side"), str):
        return False
...
def fmt_price(p):
    """Round to int string; return '?' if not a number."""
    try:
...
def extract_L1_L2(data: dict):
    """Return (L1, L2) from top-level or data['pivots']."""
...
def to_bold_unicode(s: str) -> str:
    """Bold A–Z, a–z, 0–9 using math bold; leave other chars as-is."""
    out = []
    for ch in s:
        o = ord(ch)
        if 0x41 <= o <= 0x5A:
...
def choose_tf_label(batch: list[dict]) -> str:
    """Pick tf label from the last event that provides tf_label or tf_sec."""
...
def seq_bars(seq_no: int, tf_label: str):
    """
    Build top/bottom bars. Bottom bar is padded by +2 '━' to visually match top
    in monospace consoles when using emoji / bold unicode.
    """
